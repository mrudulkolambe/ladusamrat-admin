...
const UploadComponent = ({ image, setImage}) => {
        ...
        <>
            {<div className='flex flex-col items-center w-full'>
                {image?.length === 0 ?
                    ...
                    </div> : <img src={image} onClick={() => widgetRef?.current?.open()} className='cursor-pointer border object-cover h-32 w-32 rounded-lg' />}
            </div>
            }
        </>
    )
